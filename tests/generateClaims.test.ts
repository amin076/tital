...
const approvedEvidence: EvidenceRecord[] = [
  {
    ...
    interpretation: 'The measurement supports a conductive subsurface layer consistent with liquid material.',
    strength: 'HIGH',
    uncertainty: 'The measurement supports conductivity but does not by itself establish exact composition or global extent.',
    status: 'APPROVED',
  },
];
  ...
  claims: [
    {
      ...
      uncertainty: 'The evidence supports a conductive layer; exact composition and global extent require additional evidence.',
    },
  ],
};

describe('Approved Evidence → Claim governed generation', () => {
  it('parses valid structured claim proposals without trusted evidence IDs', () => {
  ...
  it('rejects non-approved evidence before model invocation', async () => {
    ...
    await expect(generateClaims(evidence, approvedQuestion, modelCaller)).rejects.toThrow('EvidenceRecord is not approved');
  ...
  it('rejects evidence/question provenance mismatches', () => {
    ...
    expect(() => validateEvidenceForClaims(evidence, approvedQuestion)).toThrow('researchQuestionId mismatch');
  });

  it('rejects evidence numbers outside the supplied approved evidence', () => {
    expect(() =>
      assembleClaimRecords(
        approvedQuestion,
        approvedEvidence,
        { claims: [{ ...proposals.claims[0], evidenceNumbers: [2] }] },
        { idFactory: () => 'CL-fixed' }
      )
    ).toThrow('evidence number outside');
  });

  it('maps model evidence numbers to application-owned evidence IDs', () => {
